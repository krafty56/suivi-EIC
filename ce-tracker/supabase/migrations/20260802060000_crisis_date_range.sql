-- Une crise devient un épisode avec une date de début (au signalement) et
-- une date de fin, renseignée plus tard une fois la crise résolue — plutôt
-- qu'un simple jour ponctuel. Renommer préserve les crises déjà notées.
alter table public.crises rename column date to date_debut;
alter table public.crises add column if not exists date_fin date;

-- get_shared_dossier triait explicitement par crises.date : à reprendre avec
-- le nouveau nom de colonne. Le reste (to_jsonb(c)) suit sans modification.
